...
public static class Doc_Tools
{
    ...
    public static void CheckSageIntegration(int id, DocumentType type)
    {
        ...
        using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["PortalConnection"].ToString()))
        {
            ...
            {
                case DocumentType.Advance:
                    query_text = "UPDATE Advance SET SageIntegration = 1, Status = 4 WHERE AdvanceId = @id";
                    break;
                case DocumentType.Expense:
                    query_text = "UPDATE Expense SET SageIntegration = 1, Status = 4 WHERE ExpenseId = @id";
                    break;
                case DocumentType.CorporateCard:
                    query_text = "UPDATE CorporateCard SET SageIntegration = 1, Status = 4 WHERE CorporateCardId = @id";
                    break;
                case DocumentType.MinorMedicalExpense:
                    query_text = "UPDATE MinorMedicalExpense SET SageIntegration = 1, Status = 4 WHERE MinorMedicalExpenseId = @id";
                    break;
            }
            SqlCommand cmd = conn.CreateCommand();
