    ...
    private List<AdvanceDTO> ReadFromDb(int user_id)
    {
        ...
        using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["PortalConnection"].ToString()))
        {
            ...
            {                  
                ...
                advance.Status = Dict_status().FirstOrDefault(x=> x.Key == dataReader.GetInt32(13)).Value;
    ...
    private AdvanceDTO LoadAdvanceById(int advance_id, int user_id)
    {
        ...
        using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["PortalConnection"].ToString()))
        {
            ...
            while (dataReader.Read())
            {
                ...
                advance.Status = Dict_status().FirstOrDefault(x => x.Key == dataReader.GetInt32(11)).Value;
