...
public partial class Logged_Administradores_ReembolsoEmpleados : System.Web.UI.Page
{
    ...
    private int WriteToDb(int tipo_moneda, DateTime fecha_gasto, decimal importe_gasto, FileUpload fu_xml, FileUpload fu_pdf, FileUpload fu_voucher, int userkey, string companyId, int AdvanceId, List<ExpenseDetailDTO> expenseDetails, int status, string motivo_gasto)
    {
        ...
        try
        {
            ...
            {
               SqlCommand cmd = conn.CreateCommand();               
               cmd.CommandText = "Insert Into Expense (Date, Currency, Amount, FileBinaryXml, FileBinaryPdf, FileBinaryPdfVoucher, UpdateDate, UpdateUserKey, CompanyId, Status, AdvanceId, ApprovalLevel, FileNameXml, FileNamePdf, FileNamePdfVoucher, ExpenseReason) VALUES (@Date, @Currency, @Amount, @FileBinaryXml, @FileBinaryPdf, @FileBinaryPdfVoucher, @UpdateDate, @UpdateUserKey, @CompanyId, @Status, @AdvanceId, @ApprovalLevel, @FileNameXml, @FileNamePdf, @FileNamePdfVoucher, @ExpenseReason); SELECT SCOPE_IDENTITY();";
        ...
        catch (Exception ex)
        {
            if(id>0)
            {
                ...
                {
                    SqlCommand cmd = conn.CreateCommand();
                    cmd.CommandText = "DELETE FROM dbo.Expense WHERE ExpenseId = @ExpenseId;";
    ...
    private void get_anticipos(int user_id)
    {
        ...
        {
            SqlCommand cmd = conn.CreateCommand();
            cmd.CommandText = "SELECT AdvanceId , Folio FROM Advance where UpdateUserKey = @UpdateUserKey and status in (2,4);";
            cmd.Parameters.Add("@UpdateUserKey", SqlDbType.Int).Value = user_id;            
            cmd.Connection.Open();
            SqlDataReader dataReader = cmd.ExecuteReader();
            while (dataReader.Read())
